import { b as createAdapterFactory, s as sql } from "./auth-CoiYOFBV.js";
import { e, f } from "./auth-CoiYOFBV.js";
import "../server.js";
    ...
    return ({ getFieldName, schema, getDefaultFieldName, getDefaultModelName, getFieldAttributes, getModelName }) => {
      ...
      function convertWhereClause(model, w) {
        ...
        w.forEach((condition) => {
          ...
          const expr = (eb) => {
            const f2 = `${model}.${field}`;
            if (operator.toLowerCase() === "in") return eb(f2, "in", Array.isArray(value) ? value : [value]);
            if (operator.toLowerCase() === "not_in") return eb(f2, "not in", Array.isArray(value) ? value : [value]);
            if (operator === "contains") return eb(f2, "like", `%${value}%`);
            if (operator === "starts_with") return eb(f2, "like", `${value}%`);
            if (operator === "ends_with") return eb(f2, "like", `%${value}`);
            if (operator === "eq") return eb(f2, "=", value);
            if (operator === "ne") return eb(f2, "<>", value);
            if (operator === "gt") return eb(f2, ">", value);
            if (operator === "gte") return eb(f2, ">=", value);
            if (operator === "lt") return eb(f2, "<", value);
            if (operator === "lte") return eb(f2, "<=", value);
            return eb(f2, operator, value);
          };
...
export {
  e as createKyselyAdapter,
  f as getKyselyDatabaseType,
  kyselyAdapter
};
